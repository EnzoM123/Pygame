...
def chama_segunda_tela():
    ...
    try:
        ...
        print('Valor não existe ou incorreto')
...
def cadastrar():
    ...
    senha_login = tela_cadastro.lineEdit_3.text()
    c_senha = tela_cadastro.lineEdit_4.text()

    if (senha_login == c_senha):
        try:
            banco = sqlite3.connect('banco_cadastro.db')
            cursor = banco.cursor()
            cursor.execute("CREATE TABLE IF NOT EXISTS cadastro ('nome text','login text','senha text')")
            cursor.execute("INSERT INTO cadastro VALUES ('" + nome + "','" + login + "','" + senha_login + "')")
